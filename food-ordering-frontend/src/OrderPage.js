...
const OrderPage = () => {
  ...
  useEffect(() => {
    ...
    axios.get('http://localhost:8080/items')
      .then(response => {
        const filteredItems = response.data.filter(item => !item.isDelete && !item.isRemoved);
...
const handleCategoryClick = (category) => {
  ...
  const handlePlaceOrder = () => {
    ...
    const orderData = {
      ...
      order_type: 'takeaway',  // Added order type as "takeaway"
    };
...
if (orderPlaced) {
  ...
  return (
    <div className="order-page-container">
      <h1 className="page-title" lang="ar">مرحباً بك في صفحة طلبات السفري</h1>
              ...
              return (
                <li key={item.itemId}>
                  <div className="order-item">
                    <span className="item-name">{itemData?.itemName}</span>
                    <span className="item-quantity">العدد: {item.quantity}</span>
      ...
      <div className="comment-input">
        <label lang="ar">أضف تعليقاً (اختياري)</label>
        <textarea
          value={comment}
      ...
      <div className="place-order">
        <button
          ...
          disabled={loading || orderItems.filter(item => item.quantity > 0).length === 0 || !/^05\d{8}$/.test(phoneNumber)}
        >
