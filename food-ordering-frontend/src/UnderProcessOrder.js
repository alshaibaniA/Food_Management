...
const UnderProcessOrder = () => {
    const [underProcessOrders, setUnderProcessOrders] = useState([]);

    // Fetch under_process orders from the backend
    useEffect(() => {
        axios.get('http://localhost:8080/orders/under_process')
            .then(response => {
                setUnderProcessOrders(response.data);
            })
            .catch(error => {
    ...
    const handleChangeStatus = (orderId) => {
        ...
        if (orderId) {
            axios.put(`http://localhost:8080/orders/update/${orderId}`, { status: 'successful' })
                .then(response => {
                    // Remove the order from the state list after it's updated
                    setUnderProcessOrders(prevState => prevState.filter(order => order.orderId !== orderId));
                })
                .catch(error => {
    ...
    return (
        <div className="style-order-container">
            ...
            {underProcessOrders && underProcessOrders.length > 0 ? (
                ...
                <p>No under process orders available.</p>
            )}
        </div>
    );
};

export default UnderProcessOrder;
